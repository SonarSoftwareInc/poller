    ...
    private function getRemoteBackhaul(SnmpResult $snmpResult):SnmpResult
    {
        ...
        foreach ($interfaces as $id => $deviceInterface)
        {
            if (strpos($deviceInterface->getName(),"wireless") !== false)
            {
                ...
                $result = $this->device->getSnmpClient()->get("1.3.6.1.4.1.17713.5.5.4.0");
                if ($result->has("1.3.6.1.4.1.17713.5.5.4.0")) {
                    $macs[] = Formatter::formatMac($result->get("1.3.6.1.4.1.17713.5.5.4.0")->getValue()->__toString());
                }
